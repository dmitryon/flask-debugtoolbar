# This code is adapted to Flask from
# https://github.com/Pylons/pyramid_debugtoolbar/blob/master/pyramid_debugtoolbar/panels/performance.py
#
...
class PerformanceDebugPanel(DebugPanel):
    ...
    def __init__(self, jinja_env, context):
        super(PerformanceDebugPanel, self).__init__(jinja_env, context)
    ...
    def _wrap_timer_handler(self, handler):
        if self.has_resource:
            def resource_timer_handler(*args, **kw):#request):
                _start_time = time.time()
                self._start_rusage = resource.getrusage(resource.RUSAGE_SELF)
                try:
                    result = handler(*args, **kw)#request)
                except:
                    raise
                finally:
        ...
        def noresource_timer_handler(*args, **kw):#request):
            _start_time = time.time()
            try:
                result = handler(*args, **kw)#request)
            except:
    ...
    def process_view(self, request, view_func, view_kwargs):
        return self.wrap_handler(view_func)
    ...
    def content(self):
        ...
        return render_template('panels/performance.html', **vars)
